...
function signingKey(): Buffer | null {
  const secret = process.env.ROUTE_TOKEN_SECRET?.trim() || getGoogleMapsServerKey();
  if (!secret) return null;
  // Domain separation keeps these signatures independent from route tokens.
...
function isVerifiedPlace(value: unknown): value is VerifiedPlace {
  ...
  return (
    ...
    isValidPlaceId(place.providerPlaceId) &&
